...
def save_mask_crop(origin_size, mask_list, boxes_list, label_list, temp_dir, crop_dir):
    ...
    for mask, box, label in zip(mask_list, boxes_list, label_list):
        ...
        cv2.imwrite(os.path.join(crop_dir, f"{crop_idx}_{name}.png"), crop_img)
        # save original image
...
def process_image(image_path, output_dir,
    ...
    crop_dir = os.path.join(results_dir, f'crop/')
    # make dir
    ...
    save_mask_crop((H, W), masks, boxes_filt.cpu().numpy(), pred_phrases, temp_dir, crop_dir)
