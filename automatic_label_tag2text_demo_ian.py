...
def save_mask_data(output_dir, caption, mask_list, box_list, label_list):
    ...
    # make dir
    results_dir = os.path.join(output_dir, 'auto_lebal_t2t/')
    ...
    for (mask,label,box) in zip(mask_list, label_list, box_list):
        value += 1
        ...
        cv2.imwrite(
            os.path.join(img_mask_dir, f"{name}_{str_current_datetime}.png"), 
            object_img_trans
        )
