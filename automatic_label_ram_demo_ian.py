...
def save_mask_data(output_dir, tags_chinese, mask_list, box_list, label_list):
    ...
    results_dir = os.path.join(output_dir, 'auto_lebal/')
    ...
    for (mask,label) in zip(mask_list, label_list):
        ...
        print("name: ", name)
        # create directory
        # name_dir = os.path.join(results_dir, f'{name[0].lower()}/')
        temp_dir = os.path.join(results_dir, f'temp/')
        img_dir = os.path.join(results_dir, f'img/{name[0].lower()}/')
        img_mask_dir = os.path.join(results_dir, f'img_mask/{name[0].lower()}/')
        mask_dir = os.path.join(results_dir, f'mask/{name[0].lower()}/')
        ...
        plt.axis('off')
        file_name = f"{name}_{value}_mask_temp_{str_current_datetime}.png"
        ...
        plt.axis('off')
        color_file_name = f"{name}_{value}_mask_{str_current_datetime}.png"
        os.makedirs(mask_dir, exist_ok=True)
        plt.savefig(
            os.path.join(mask_dir, color_file_name),
            bbox_inches="tight", pad_inches=0.0, transparent=True
        )

        ### object + color mask without background ###
        mask_temp = cv2.imread(os.path.join(
            temp_dir, f"{name}_{value}_mask_temp_{str_current_datetime}.png")
        )
        ...
        cv2.imwrite(
            os.path.join(img_mask_dir, f"{name}_{value}_mask_img_{str_current_datetime}.png"), 
            object_img_trans
        )

        ### object without background ###
        raw_img = cv2.imread(os.path.join(temp_dir, f"raw_image.png"))
        object_img = cv2.cvtColor(raw_img, cv2.COLOR_BGR2BGRA)
        object_img[:, :, 3] = mask_invert
        os.makedirs(img_dir, exist_ok=True)
        cv2.imwrite(
            os.path.join(img_dir, f"{name}_{value}_img_{str_current_datetime}.png"),
              object_img
        )
...
if __name__ == "__main__":
    ...
    os.makedirs(temp_dir, exist_ok=True)
    plt.savefig(
        os.path.join(temp_dir, "raw_image.png"),
        bbox_inches="tight", pad_inches=0.0
    )
    ...
    # draw output image with label and box
    plt.figure(figsize=(10, 10))
    plt.imshow(image)
    for mask in masks:
